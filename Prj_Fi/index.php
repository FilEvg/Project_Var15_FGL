                        ...
                        <div class="row text-center">
                            ...
                            foreach ($stats as $stat) {
                                ...
                                echo '<div class="col-md-3">
                                        <h3>' . htmlspecialchars($row['total'] ?? $row['count'], ENT_QUOTES, 'UTF-8') . '</h3>
                                        <p class="text-muted">' . htmlspecialchars($stat['label'], ENT_QUOTES, 'UTF-8') . '</p>
                                      </div>';
                            }
                            ?>
                        </div>
...
function page_backup() {
    // Только администраторы и пользователи с правами
    if (!isLoggedIn() || !hasPermission('export_reports')) {
        $_SESSION['error'] = 'Доступ запрещен. Недостаточно прав.';
    ...
    switch ($action) {
        case 'create':
            if (in_array($format, ['csv', 'json', 'sql', 'xml'])) {
        ...
        case 'delete':
            if ($file && deleteBackup($file)) {
                $message = '<div class="alert alert-success">Файл успешно удален</div>';
            } elseif ($file) {
                $message = '<div class="alert alert-danger">Ошибка при удалении файла</div>';
            }
            break;
        ...
        case 'restore':
            if ($file && file_exists(BACKUP_DIR . $file)) {
                ...
                $result = restoreFromBackup(BACKUP_DIR . $file, $truncate);
                $message = $result['success']
                    ? '<div class="alert alert-success">' . $result['message'] . '</div>'
                    : '<div class="alert alert-danger">' . $result['message'] . '</div>';
            }
    ...
    displayHeader('Резервное копирование');
    ?>
    <div class="container mt-4">
        <h2><i class="bi bi-database"></i> Резервное копирование данных</h2>
            ...
            <div class="col-md-4">
                <div class="card">
                    <div class="card-header bg-primary text-white">
                        <i class="bi bi-plus-circle"></i> Создать резервную копию
                    </div>
                    <div class="card-body">
                        <p>Выберите формат для создания резервной копии:</p>
                        
                        <div class="d-grid gap-2">
                            <a href="?page=backup&action=create&format=csv" class="btn btn-info text-white">
                                <i class="bi bi-file-earmark-spreadsheet"></i> CSV (совместим с Excel)
                            </a>
                            <a href="?page=backup&action=create&format=json" class="btn btn-warning text-dark">
                                <i class="bi bi-file-earmark-code"></i> JSON (структурированный)
                            </a>
                            <a href="?page=backup&action=create&format=sql" class="btn btn-danger text-white">
                                <i class="bi bi-database"></i> SQL (дамп БД)
                            </a>
                            <a href="?page=backup&action=create&format=xml" class="btn btn-success text-white">
                                <i class="bi bi-file-earmark-xml"></i> XML (универсальный)
                            </a>
                        </div>
                        ...
                        <div class="alert alert-info">
                            ...
                            <strong>Информация о форматах:</strong><br>
                            <small>
                            - <b>CSV</b>: можно открыть в Excel, занимает мало места<br>
                            - <b>JSON</b>: для програмной обработки, сохраняет структуру<br>
                            - <b>SQL</b>: полный дамп базы, можно восстановить через phpMyAdmin<br>
                            - <b>XML</b>: универсальный формат для обмена данными
                            </small>
                        </div>
                    </div>
                </div>
                ...
                <div class="card mt-3">
                    <div class="card-header bg-secondary text-white">
                        <i class="bi bi-gear"></i> Настройки восстановления
                    </div>
                    <div class="card-body">
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" id="truncateBeforeRestore" checked>
                            <label class="form-check-label" for="truncateBeforeRestore">
                                Очищать таблицы перед восстановлением
                            </label>
                        </div>
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
                        <div>
                            <i class="bi bi-list"></i> Доступные резервные копии
                        </div>
                        <span class="badge bg-light text-dark">Всего: <?php echo count($backups); ?></span>
                    </div>
                    <div class="card-body">
                        <?php if (empty($backups)): ?>
                            <div class="alert alert-warning">
                                <i class="bi bi-exclamation-triangle"></i> 
                                Нет доступных резервных копий. Создайте первую копию.
                            </div>
                        <?php else: ?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge bg-<?php 
                                                    echo $backup['format'] == 'CSV' ? 'info' : 
                                                        ($backup['format'] == 'JSON' ? 'warning' : 
                                                        ($backup['format'] == 'SQL' ? 'danger' : 'success')); 
                                                ?>">
                                                    <?php echo $backup['format']; ?>
                                                </span>
                                            </td>
                                            <td>
                                                <?php if ($backup['tables'] != '?'): ?>
                                                    <small><?php echo $backup['tables']; ?> / <?php echo $backup['rows']; ?></small>
                                                <?php else: ?>
                                                    <small>-</small>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <div class="btn-group btn-group-sm" role="group">
                                                    <a href="?page=backup&action=download&file=<?php echo urlencode($backup['filename']); ?>" 
                                                       class="btn btn-primary" title="Скачать">
                                                        <i class="bi bi-download"></i>
                                                    </a>
                                                    
                                                    <button type="button" class="btn btn-warning" 
                                                            data-bs-toggle="modal" 
                                                            data-bs-target="#restoreModal<?php echo $index; ?>"
                                                            title="Восстановить">
                                                        <i class="bi bi-arrow-repeat"></i>
                                                    </button>
                                                    
                                                    <a href="?page=backup&action=delete&file=<?php echo urlencode($backup['filename']); ?>" 
                                                ...
                                                <div class="modal fade" id="restoreModal<?php echo $index; ?>" tabindex="-1">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <p><strong>Внимание!</strong> Восстановление из резервной копии заменит текущие данные.</p>
                                                                <p>Файл: <code><?php echo htmlspecialchars($backup['filename'], ENT_QUOTES, 'UTF-8'); ?></code></p>
                                                                <p>Дата создания: <?php echo $backup['date']; ?></p>
                                                                <p>Размер: <?php echo $backup['size']; ?></p>
                                                                <p>Формат: <?php echo $backup['format']; ?></p>
                                                                
                                                                <div class="alert alert-danger mt-3">
                                                                    <i class="bi bi-info-circle"></i>
                                                                    Рекомендуется создать текущую резервную копию перед восстановлением.
                                                                </div>
                                                            </div>
                                                            <div class="modal-footer">
                                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Отмена</button>
                                                                <a href="?page=backup&action=restore&file=<?php echo urlencode($backup['filename']); ?>&truncate=1" 
                                                                   class="btn btn-warning" 
                                                                   onclick="return confirm('Вы уверены? Это действие необратимо!')">
                                                                    <i class="bi bi-arrow-repeat"></i> Восстановить данные
                                                                </a>
                                                            </div>
                                                        </div>
                                                    </div>
                ...
                <!-- Информация о последних операциях -->
                <?php if (file_exists(BACKUP_DIR . 'backup_log.txt')): ?>
                <div class="card mt-3">
                    <div class="card-header bg-info text-white">
                        <i class="bi bi-journal-text"></i> Журнал операций
                    </div>
                    <div class="card-body">
                        <div class="log-viewer" style="max-height: 200px; overflow-y: auto; background: #f8f9fa; padding: 10px; border-radius: 5px; font-size: 0.85rem;">
                            <?php
                            if (file_exists(BACKUP_DIR . 'backup_log.txt')) {
                                $logs = file(BACKUP_DIR . 'backup_log.txt');
                                $logs = array_slice($logs, -20);
                                foreach (array_reverse($logs) as $log) {
                                    echo '<div>' . htmlspecialchars($log) . '</div>';
                                }
                            } else {
                                echo '<div class="text-muted">Нет записей в журнале</div>';
                            }
                            ?>
                        </div>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Передаём состояние чекбокса в ссылки восстановления
        const truncateCheckbox = document.getElementById('truncateBeforeRestore');
        
        document.querySelectorAll('[data-bs-target^="#restoreModal"]').forEach(button => {
            button.addEventListener('click', function() {
                const modalId = this.getAttribute('data-bs-target');
                const restoreLink = document.querySelector(modalId + ' .modal-footer a');
                if (restoreLink) {
                    let href = restoreLink.getAttribute('href');
                    href = href.replace(/&truncate=\d/, '&truncate=' + (truncateCheckbox.checked ? 1 : 0));
                    restoreLink.setAttribute('href', href);
                }
            });
        });
    });
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($product['internal_code'], ENT_QUOTES, 'UTF-8'); ?></td>
                            ...
                            <table class="table table-hover table-sm">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($sale['subdivision_name'], ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td><small><?php echo htmlspecialchars($sale['product_name'], ENT_QUOTES, 'UTF-8'); ?></small></td>
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><span class="badge bg-<?php echo htmlspecialchars($role_color, ENT_QUOTES, 'UTF-8'); ?>">
                                        <?php echo htmlspecialchars($user['role_name'] ?: 'Не назначена', ENT_QUOTES, 'UTF-8'); ?>
                                    </span></td>
